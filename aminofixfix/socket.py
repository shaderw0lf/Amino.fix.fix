...
class SocketHandler:
    def __init__(
        ...
        self.socket_url = socket_url or f"wss://ws{randint(1, 4)}.aminoapps.com"
        self.is_custom_url = socket_url is not None
        self.debug = debug
        self.socket = None
        self.active = False
        self.headers = None
    ...
    def socket_log(self, text, status: str = "INFO"):
        if self.debug is True:
            print(
                "[SOCKET: {}] ({})".format(
                    status, dt.now().strftime("%Y-%m-%d %H:%M:%S")
                ),
                text,
            )
    ...
    def close(self):
        ...
        try:
            ...
            self.socket_log(
                "Can't close connection to {}: {}".format(
                    self.socket_url, str(closeError).replace("\n", " ")
                ),
                "ERROR",
            )
