...
class SubClient(Client):
    ...
    async def _init(self):
        ...
        if self.__get_profile:
            try:
                self.profile: objects.UserProfile = await self.get_user_info(
    ...
    async def post_blog(
        ...
        if captionList is not None:
            for image, caption in zip(imageList, captionList):
                mediaList.append(
                    [100, await self.upload_media(image, "image"), caption]
                )
            ...
            if imageList is not None:
                for image in imageList:
                    mediaList.append(
                        [100, await self.upload_media(image, "image"), None]
                    )
    ...
    async def subscribe(
        self, userId: str, autoRenew: str = False, transactionId: str = None
    ):
        if transactionId is None:
            transactionId = str_uuid4()
    ...
    async def get_public_chat_threads(
    ...
    async def get_shared_folder_files(
    ...
    async def strike(
        self, userId: str, time: int, title: str = None, reason: str = None
    ):
        if time == 1:
            time = 3600
        elif time == 2:
    ...
    async def ban(self, userId: str, reason: str, banType: int = None):
        data = dumps(
            {"reasonType": banType, "note": {"content": reason}, "timestamp": inttime()}
        )
    ...
    async def accept_wiki_request(
        self, requestId: str, destinationCategoryIdList: list
    ):
        data = dumps(
            {
