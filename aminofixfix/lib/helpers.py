...
def gen_deviceId(data: bytes = None) -> str:
    identifier = PREFIX + (
        (data if isinstance(data, bytes) else bytes(data, "utf-8"))
        if data
        else urandom(20)
    )
    return "{}{}".format(
        identifier.hex(), new(DEVICE_KEY, identifier, sha1).hexdigest()
    ).upper()
...
def decode_sid(sid: str) -> dict:
    return loads(
        b64decode(
            reduce(
                lambda a, e: a.replace(*e), ("-+", "_/"), sid + "=" * (-len(sid) % 4)
            ).encode()
        )[1:-20].decode()
    )
